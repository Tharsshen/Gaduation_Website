...
function animate() {
  ...
  if (keys.right.pressed && player.position.x < 400) {
    ...
    if (keys.right.pressed) {
      ...
      hatObject.position.x -= player.speed;
  ...
  if (player.position.y + player.height <= hatObject.position.y && player.position.y + player.height + player.velocity.y >= hatObject.position.y && player.position.x + player.width >= hatObject.position.x && player.position.x <= hatObject.position.x + hatObject.width) {
    // collect the hat
    hatObject.collected = true;
  } // win condition


  if (scrollOffset > platformImage.width * 10 + 750 - 2) {
    console.log('You win');
  } // lose condition
